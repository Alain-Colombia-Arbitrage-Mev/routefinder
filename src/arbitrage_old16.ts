...
async function calcArbitrage(g: Graph): Promise<ArbitrageRoute[]> {
  ...
  for (const originToken of ORIGIN_TOKENS) {
    ...
    for (const cycle of cycles) {
      ...
      if (!uniqueCycle[cycleString]) {
        ...
        if (cycleWeight > 1.015 && cycleWeight < 1.5 && hasUniswap && hasQuickSwap) {
          arbitrageData.push({
            cycle: cycle,
            cycleWeight: cycleWeight,
            steps: steps.map(step => ({
              ...step,
              feeTier: step.feeTier || (step.dex === DEX.QuickSwap ? 3000 : undefined) // Include feeTier, use 3000 for QuickSwap
            })),
            type: 'cyclic'
          });
        }
  ...
  const processedArbitrageData = arbitrageData.map(route => ({
    ...
    steps: route.steps.map(step => ({
      ...
      feeTier: step.feeTier // Ensure feeTier is included
    }))
  }));
...
async function main(numberTokens: number = 5, DEXs: Set<DEX>, debug: boolean = false) {
  try {
    ...
    if (DEXs.has(DEX.QuickSwap)) {
      quickPools = await fetchQuickSwapPools(quickTokens, 50);
