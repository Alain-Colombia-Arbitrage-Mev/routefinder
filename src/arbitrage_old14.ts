...
const BLACKLISTED_TOKENS = [
  ...
  // Añade aquí más tokens que quieras poner en la lista negra
];
...
async function fetchTokens(first: number, skip: number = 0, dex: DEX, minVolumeUSD: number = 50000): Promise<string[]> {
  let dexEndpoint = (dex === DEX.UniswapV3) ? UNISWAP.ENDPOINT : SUSHISWAP.ENDPOINT;
  let tokensQuery = (dex === DEX.UniswapV3) 
    ? UNISWAP.HIGHEST_VOLUME_TOKENS(first, skip, minVolumeUSD) 
    : SUSHISWAP.HIGHEST_VOLUME_TOKENS(first, skip, minVolumeUSD);
  
  try {
    let response = await request(dexEndpoint, tokensQuery);
    let tokens = response.tokens;
    
    console.log(`Tokens from ${dex} with volume > $${minVolumeUSD}:`);
    tokens.forEach((t: any) => {
      console.log(`${t.symbol} (${t.id}): $${parseFloat(t.volumeUSD).toFixed(2)}`);
    });

    return tokens
      .map((t: any) => t.id)
      .filter(token => !BLACKLISTED_TOKENS.includes(token));
  } catch (error) {
  ...
  for (var pool of Array.from(pools.values())) {
    try {
      ...
      if (dex === DEX.UniswapV3) {
        ...
        if (price === 0 || BLACKLISTED_TOKENS.includes(token0) || BLACKLISTED_TOKENS.includes(token1)) continue;
  ...
  function dfs(currentToken: string, path: string[], weight: number, steps: SwapStep[]) {
    ...
    if (currentToken === targetToken && path.length >= minLength) {
      ...
      if (cycleWeight > 1.015 && cycleWeight < 1.5 && cycleWeight > bestWeight) {
        ...
        (bestArbitrage as any)[`step${steps.length + 1}`] = {
          ...
          poolAddress: '' // This should be filled with actual pool address if available
        };
      }
      return;
    }
    ...
    for (const edge of edges) {
      ...
      if (!path.includes(nextToken)) {
        const fee = edge.metadata.dex === DEX.UniswapV3 
          ? edge.metadata.fee 
          : 0.003;
...
async function calcArbitrage(g: Graph): Promise<ArbitrageRoute[]> {
  ...
  for (const originToken of ORIGIN_TOKENS) {
    if (BLACKLISTED_TOKENS.includes(originToken)) continue;
        ...
        if (cycleWeight > 1.015 && cycleWeight < 1.5 && hasUniswap && hasSushiswap) {
          ...
          steps.forEach((step, index) => {
            (arbitrageRoute as any)[`step${index + 1}`] = {
              ...
              feeTier: step.feeTier || (step.dex === DEX.Sushiswap ? 3000 : undefined)
            };
          });
...
function findCycles(g: Graph, startVertex: GraphVertex, minLength: number, maxLength: number): string[][] {
  ...
  function dfs(currentVertex: GraphVertex, depth: number) {
    ...
    for (const edge of edges) {
      ...
      } else if (!visited[nextVertex.getKey()] && depth < maxLength - 1 && !BLACKLISTED_TOKENS.includes(nextVertex.getKey())) {
...
async function main(numberTokens: number = 5, DEXs: Set<DEX>, debug: boolean = false, minVolumeUSD: number = 500000) {
  try {
    console.log("Iniciando el proceso de arbitraje...");

    let uniTokens = DEXs.has(DEX.UniswapV3) ? await fetchTokens(numberTokens, 0, DEX.UniswapV3, minVolumeUSD) : [];
    let sushiTokens = DEXs.has(DEX.Sushiswap) ? await fetchTokens(numberTokens, 0, DEX.Sushiswap, minVolumeUSD) : [];
    
    let tokenIds = [...new Set([
      ...uniTokens, 
      ...sushiTokens, 
      ...ORIGIN_TOKENS.filter(token => !BLACKLISTED_TOKENS.includes(token))
    ])];
...
if (require.main === module) {
  main(10, new Set([DEX.UniswapV3, DEX.Sushiswap]), true, 500000)
