    ...
    const BLACKLISTED_TOKENS = [
      // Añade aquí las direcciones de los tokens que quieres excluir
    ];
    ...
    async function fetchTokens(first: number, skip: number = 0, dex: DEX, minVolumeUSD: number = 100000): Promise<string[]> {
    ...
    function calculateCycleWeight(g: Graph, cycle: string[]): { weight: number, steps: SwapStep[] } {
      ...
      for (let i = 0; i < cycle.length - 1; i++) {
        ...
        if (edge) {
          ...
          : 3000  // Use 3000 (0.3%) for Sushiswap
          });
        } else {
          console.warn(`No edge found between ${cycle[i]} and ${cycle[i + 1]}`);
    ...
    async function main(numberTokens: number = 5, DEXs: Set<DEX>, debug: boolean = false, minVolumeUSD: number = 50000) {
  try {
...
if (require.main === module) {
  main(10, new Set([DEX.UniswapV3, DEX.Sushiswap]), true, 50000)
